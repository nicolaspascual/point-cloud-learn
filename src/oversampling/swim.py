...
class SWIM:
    ...
    def sample(self, data, labels, numSamples=None):

        if not numSamples:
            numSamples = max(np.bincount(labels.astype(int))) - \
                min(np.bincount(labels.astype(int)))
        ...
        if data_rank < T_maj.shape[0]:
            if self.subSpaceSampling == False:
                ...
                data = data[:, indep]
                print("The majority class has linearly dependent columns. Resampled data will be in the " + str(sum(indep ==
                                                                                                                    True)) + " independent columns of the orginal " + str(data_maj_orig.shape[1]) + "-dimensional data.")
            ...
            try:
                ...
                if 'Singular matrix' in str(e):
                    ...
                    X_new = data_min_orig[np.random.choice(
                        data_min_orig.shape[0], numSamples, replace=True), :]
                    X_new = X_new + (0.1 * np.random.normal(0,
                                                            data_maj_orig.std(0), X_new.shape))
        ...
        try:
            ...
            # whitening transform - whiten the minority class
            W_min = M.dot(T_min)
            # whitening transform - whiten the majority class
            W_maj = M.dot(T_maj)
        except:
        ...
        for smpInd in range(smpInitPts.shape[1]):
            ...
            for dim in range(len(min_means)):
                new_w_raw.append(random.uniform(
                    smp[dim]-self.sd*min_stds[dim], smp[dim]+self.sd*min_stds[dim]))
        ...
        sampled_labels = np.append(
            [self.minClass]*len(syntheticInstances), labels)
